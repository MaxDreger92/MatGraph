...
class ImportingReport(models.Model):
    ...
    def save(self, force_insert=False, force_update=False, using=None, update_fields=None, save_to_file_server=True):
    ...
    def delete(self, force_insert=False, force_update=False, using=None, update_fields=None, save_to_file_server=True):
...
class Cache:
    ...
    def fetch(cls, header, column_value, attribute_type):
        ...
        if cached:
            ...
            new_record = cls.objects.get_or_create(header=str(header)[:200])[0]
...
class FullTableCache(models.Model, Cache):
    ...
    def fetch(cls, header):
        ...
        if cached:
            ...
            new_record = cls.objects.create(header=header)
            new_record.save()
