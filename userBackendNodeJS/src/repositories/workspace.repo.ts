...
class WorkspaceRepository {
    static getGraphCollection = async () => {
        ...
        const collections = await db.listCollections({ name: graphCollection }).toArray()
        if (collections.length === 0) {
            await db.createCollection(graphCollection)
        }
    ...
    static getUploadCollection = async () => {
        ...
        const collections = await db.listCollections({ name: uploadCollection }).toArray()
        if (collections.length === 0) {
            await db.createCollection(uploadCollection)
        }
